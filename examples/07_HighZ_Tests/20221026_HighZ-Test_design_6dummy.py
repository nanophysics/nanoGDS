...
CPW_RADIUS = 400
FILLET_RADIUS = 10
SAVENAME = "20221026_HighZ-Test_design_6dummy"
...
if __name__ == "__main__":
    ...
    coupling_lengths = [80 for i in range(6)]
    resonator_lengths = [800 - 30 * i for i in range(6)]
    resonator_widths = [0.75, 0.75, 0.75, 0.75, 0.75, 0.75]
    text = ""
    for i in range(6):
...
#        )
        feedline.combine(
            resonator,
            position=[(i - 2.5) * 600, -FEEDLINE_WIDTH / 2 - CPW_GAP],
            connect_point=resonator.points["END"],
        )
        feedline.combine(
            get_coupler_shape(coupling_lengths[i], resonator_widths[i]), position=[(i - 2.5) * 600, 0]
        )
